...
from local_pattern_cell_rules import captured_cells, dead_cells
...
PACKED_DEAD_CELL = 1001
PACKED_CAPTURED_CELL = 1002
...
def _website_entry_from_tile(tile: dict[str, Any]) -> dict[str, Any]:
    ...
    dead = dead_cells(parsed.plus, parsed.minus)
    captured = captured_cells(parsed.plus, parsed.minus)
    ...
    for cell in tile[tile_keys["cells"]]:
        ...
        if local is not None:
            if tuple(local) in dead:
                fraction_code = PACKED_DEAD_CELL
            elif tuple(local) in captured:
                fraction_code = PACKED_CAPTURED_CELL
            else:
                fraction_code = encode_thousandths(stone_fraction, clamp=True)
            local_cells.append(
                [
                    int(local[0]),
                    int(local[1]),
