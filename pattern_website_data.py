...
FRACTION_MODE_ROW_COORD_BITPLANE = 3
...
def _encode_index_gaps(indices: list[int]) -> bytes:
    out = bytearray()
    out.extend(encode_uvarint(len(indices)))
    prev = -1
    for idx in indices:
        out.extend(encode_uvarint(int(idx) - prev - 1))
        prev = int(idx)
    return bytes(out)
...
def _encode_presence_stream(
    ...
    bitmap_byte_length = (len(pair_rows) + 7) // 8
    for cell_map in entry_cell_maps:
        present_pairs = set(cell_map)
        current = [0] * len(pair_rows)
        for pair in present_pairs:
            current[pair_index[pair]] = 1
        changed = [idx for idx, (left, right) in enumerate(zip(previous, current)) if left != right]
        changed_set = set(changed)
        gap_stream = _encode_index_gaps(changed)
        bitmap_stream = pack_little_endian_bits(
            [(1 if idx in changed_set else 0, 1) for idx in range(len(pair_rows))],
            chunk_bytes=4,
        )
        if len(gap_stream) <= bitmap_byte_length:
            flags.append((0, 1))
            payload.extend(gap_stream)
        else:
            flags.append((1, 1))
            payload.extend(bitmap_stream)
        previous = current
    return pack_little_endian_bits(flags, chunk_bytes=4) + bytes(payload)
...
def _build_row_coord_fraction_stream(
    ...
    out = bytearray()
    out.extend(_pack_bitplanes(row_means, PACKED_STONE_FRACTION_BITS))
...
def _build_pattern_bundle_from_index(index: dict[str, Any]) -> bytes:
    ...
    out.extend(
        HEADER_STRUCT.pack(
            BUNDLE_MAGIC,
            BUNDLE_VERSION,
            FRACTION_MODE_ROW_COORD_BITPLANE,
            len(entry_cell_maps),
            cell_count,
            len(key_stream),
        )
    ...
    out.extend(_build_row_coord_fraction_stream(entry_cell_maps, pair_rows))
