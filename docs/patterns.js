const MANIFEST_URL = "./data/patterns_current.json"
const BUNDLE_MAGIC = "HPB"
...
const PACKED_DEAD_CELL = 1001
const PACKED_CAPTURED_CELL = 1002
const FRACTION_MODE_AFFINE_ROW_COORD_BITPLANE = 3
const ROW_MEAN_PREDICTOR_SIZE = 5
...
const elements = {
  ...
  random6Btn: document.getElementById("random-6-btn"),
...
const state = {
  manifest: null,
  dataLayers: [],
  loadingPromiseByLayerId: new Map(),
  errorByLayerId: new Map(),
  dataError: null,
  inputError: null,
  board: {
    red: [CENTER_POINT],
...
const randomBucketControls = createModeButtonGroup({
  state,
  field: "randomBucket",
  values: ["le4", "5", "6"],
  rows: [
    ...
    ["6", elements.random6Btn],
  ],
  defaultValue: "le4",
  render: () => syncRandomModeUi(),
})
...
function setBoardState({ red, blue, toPlay, tenukiOwner = null }) {
  cancelBoardInteractionForBoardChange()
  state.board = {
    red: sortPoints(red),
...
function readSignedBitplaneValue(view, offset, valueCount, valueIndex, bits) {
  return unzigzag(readBitplaneValue(view, offset, valueCount, valueIndex, bits))
}

function roundRatioHalfEven(numerator, denominator) {
  assertPatternBundleSize(Number.isInteger(numerator) && Number.isInteger(denominator) && denominator > 0)
  let quotient = Math.floor(numerator / denominator)
  const doubledRemainder = (numerator - (quotient * denominator)) * 2
...
function decodePresenceRows(bytes, patternCount, pairTable) {
  const pairCount = pairTable.length
  const maskByteLength = Math.ceil(pairCount / 8)
  const flagByteLength = Math.ceil(maskByteLength / 8)
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength)
  const coordRows = pairTable.map((pair) => ({
    pair,
    prefix: new Uint32Array(patternCount + 1),
    fractionStart: 0,
  }))
  const previous = new Uint8Array(maskByteLength)
  let offset = 0
  for (let entryIdx = 0; entryIdx < patternCount; entryIdx += 1) {
    const flagOffset = offset
    offset += flagByteLength
    assertPatternBundleSize(offset <= bytes.length)
    for (let byteIdx = 0; byteIdx < maskByteLength; byteIdx += 1) {
      if (readPackedWordAtBit(view, flagOffset, byteIdx, 1) === 1) {
        assertPatternBundleSize(offset < bytes.length)
        previous[byteIdx] ^= bytes[offset]
        offset += 1
      }
    }
    for (let pairIdx = 0; pairIdx < pairCount; pairIdx += 1) {
      const prefix = coordRows[pairIdx].prefix
      const present = (
        previous[Math.floor(pairIdx / 8)] >> (pairIdx % 8)
      ) & 1
      prefix[entryIdx + 1] = prefix[entryIdx] + present
    }
  }
  assertPatternBundleSize(offset === bytes.length)
  let fractionStart = 0
  for (const row of coordRows) {
    row.fractionStart = fractionStart
    fractionStart += row.prefix[patternCount]
  }
  return { coordRows, cellCount: fractionStart }
}
...
function normalizeLoadedData(rawBuffer) {
  ...
  const fractionMode = view.getUint16(3, true)
  if (fractionMode !== FRACTION_MODE_AFFINE_ROW_COORD_BITPLANE) {
    throw new Error("Unsupported pattern fraction stream")
  }
  const patternCount = view.getUint32(5, true)
  const cellCount = view.getUint32(9, true)
  const keyStreamSize = view.getUint32(13, true)
  ...
  const keyStreamOffset = rawBuffer.byteLength - keyStreamSize
  assertPatternBundleSize(keyStreamOffset >= fractionOffset)
  const pairTable = []
  ...
  const decodedPresence = decodePresenceRows(presenceBytes, patternCount, pairTable)
  const coordRows = decodedPresence.coordRows
  assertPatternBundleSize(decodedPresence.cellCount === cellCount)
  const fractionData = parseFractionData(view, fractionOffset, keyStreamOffset, patternCount, pairCount, cellCount)
  ...
  for (let idx = 0; idx < patternCount; idx += 1) {
    ...
    entries.push({
      toPlay: compactPatternKeyToPlay(keyBytes),
      tenukiStoneFraction,
    })
...
function parseFractionData(view, fractionOffset, keyStreamOffset, patternCount, pairCount, cellCount) {
  assertPatternBundleSize(fractionOffset + ROW_MEAN_PREDICTOR_SIZE <= keyStreamOffset)
  const rowMeanNumerator = view.getUint8(fractionOffset)
  const rowMeanDenominator = view.getUint8(fractionOffset + 1)
  const rowMeanIntercept = view.getInt16(fractionOffset + 2, true)
  const rowMeanResidualMagnitudeBits = view.getUint8(fractionOffset + 4)
  assertPatternBundleSize(rowMeanDenominator > 0 && rowMeanResidualMagnitudeBits > 0)
  const rowMeanResidualOffset = fractionOffset + ROW_MEAN_PREDICTOR_SIZE
  let offset = rowMeanResidualOffset + Math.ceil(
    (patternCount * (rowMeanResidualMagnitudeBits + 1)) / 8,
  )
  const pairMeanOffset = offset
  offset += Math.ceil((pairCount * PACKED_STONE_FRACTION_BITS) / 8)
  assertPatternBundleSize(offset + 3 <= keyStreamOffset)
  const globalMean = view.getUint16(offset, true)
  offset += 2
  const residualBits = view.getUint8(offset)
  offset += 1
  const residualByteLength = Math.ceil((cellCount * residualBits) / 8)
  assertPatternBundleSize(offset + residualByteLength === keyStreamOffset)
  return {
    rowMeanNumerator,
    rowMeanDenominator,
    rowMeanIntercept,
    rowMeanResidualMagnitudeBits,
    rowMeanResidualOffset,
    pairMeanOffset,
    globalMean,
    residualBits,
...
function patternEntryForLookupInData(data, pattern) {
  ...
  const cells = []
  const rowMeanResidualCode = readBitplaneValue(
    view,
    data.fractionData.rowMeanResidualOffset,
    data.pattern_count,
    entryIndex,
    data.fractionData.rowMeanResidualMagnitudeBits + 1,
  )
  const rowMeanSignBit = 2 ** data.fractionData.rowMeanResidualMagnitudeBits
  const rowMeanMagnitude = rowMeanResidualCode % rowMeanSignBit
  const rowMeanResidual = rowMeanResidualCode >= rowMeanSignBit
    ? -rowMeanMagnitude
    : rowMeanMagnitude
  const rowMean = roundRatioHalfEven(
    data.fractionData.rowMeanNumerator * rawEntry.tenukiStoneFraction,
    data.fractionData.rowMeanDenominator,
  ) + data.fractionData.rowMeanIntercept + rowMeanResidual
  assertPatternBundleSize(rowMean >= 0 && rowMean < PACKED_OPTIONAL_NULL)
  for (let rowIdx = 0; rowIdx < data.coordRows.length; rowIdx += 1) {
    const row = data.coordRows[rowIdx]
    if (row.prefix[entryIndex + 1] === row.prefix[entryIndex]) {
      continue
    }
    ...
    const pairMean = readBitplaneValue(
      view,
      data.fractionData.pairMeanOffset,
      data.coordRows.length,
      rowIdx,
      PACKED_STONE_FRACTION_BITS,
    )
    const residual = readSignedBitplaneValue(
      view,
      data.fractionData.residualOffset,
      data.cellCount,
      fractionIndex,
      data.fractionData.residualBits,
    )
    const stoneFraction = rowMean + pairMean - data.fractionData.globalMean + residual
    assertPatternBundleSize(stoneFraction >= 0 && stoneFraction < PACKED_OPTIONAL_NULL)
    const pair = row.pair
...
function patternEntryForLookup(pattern, layerId) {
  for (const row of state.dataLayers) {
    if (String(row.id) !== String(layerId)) {
      continue
    }
    const entry = patternEntryForLookupInData(row.data, pattern)
    if (entry) {
      return entry
    }
  }
...
function candidateOverlayForLookup(entry, canonical) {
  ...
  for (const cell of entry.c || []) {
    ...
    const unavailable = cell[2] === PACKED_DEAD_CELL || cell[2] === PACKED_CAPTURED_CELL
    overlay.push({
      stoneFraction: unavailable ? 0 : decodeThousandths(cell[2]),
...
function loadedPatternsLabel() {
  const total = totalPatternCount()
  return `${formatVisits(total)} pattern${total === 1 ? "" : "s"} in corpus`
}

function loadedPatternCount() {
  return state.dataLayers.reduce((sum, row) => sum + Number(row?.data?.pattern_count || 0), 0)
}

function totalPatternCount() {
  return Number(state.manifest?.pattern_count || loadedPatternCount() || 0)
}

function isPatternLayerLoaded(layerId) {
  return state.dataLayers.some((row) => String(row.id) === String(layerId))
}

function patternLayerIdForMoveCount(moveCount) {
  return moveCount === 6 ? "m6" : "base"
}

function patternLayerLoadFailed(layerId) {
  return Boolean(state.dataError || state.errorByLayerId.has(layerId))
}

function patternLayerLoadingLabel(layerId) {
  return layerId === "m6" ? "Loading 6-move patterns..." : "Loading patterns…"
}

function syncPatternLoadStatus() {
  const count = loadedPatternCount()
  elements.patternsCount.textContent = count > 0 ? loadedPatternsLabel() : ""
}
...
function syncFromBoard({ rewriteInput = false } = {}) {
  ...
  setCopyButtonValue(elements.canonicalPattern, canonical.pattern)
  syncPatternLoadStatus()
  setTurnStatus(elements.turnIndicator, boardState.toPlay)
  if (!state.inputError) {
  ...
  const moveCount = moveCountForPattern(canonical.pattern)
  const layerId = patternLayerIdForMoveCount(moveCount)
  const entry = patternEntryForLookup(canonical.pattern, layerId)
  if (entry) {
  ...
  } else if (patternLayerLoadFailed(layerId)) {
    elements.lookupStatus.textContent = "Pattern bundle failed to load."
  } else if (!isPatternLayerLoaded(layerId)) {
    elements.lookupStatus.textContent = patternLayerLoadingLabel(layerId)
  } else {
...
function patternDragData(point) {
  if (pointAtCenter(point) || pointAtTenuki(point)) {
    return null
  }
  const stone = occupiedMap().get(pointKey(point[0], point[1])) || null
  return stone && !stone.fixed ? { sourceColor: stone.color } : null
}
...
function beginPatternDrag(interaction) {
  state.drag = {
...
function movePatternDrag(_interaction, point) {
  if (!state.drag) {
    return
  }
  const occupied = occupiedMap()
  const nextTarget = dragTargetFromPoint(state.drag.startPoint, point, occupied)
...
function commitBoardChange() {
  ...
  void preloadAllPatternLayers()
}

function activateBoardPoint(point) {
  if (point === null || pointAtCenter(point)) {
    return
  }
  const boardState = boardPatternState()
  if (pointAtTenuki(point)) {
  ...
  const stone = occupiedMap().get(pointKey(point[0], point[1])) || null
  if (stone) {
    if (!stone.fixed && stone.color === removableStoneColor()) {
      removeStoneAtPoint(point)
      commitBoardChange()
    }
    return
  }
  ...
  if (boardState.toPlay === "red") {
    setBoardState({
      red: sortPoints([...boardState.red, [point[0], point[1]]]),
      blue: boardState.blue,
      toPlay: "blue",
      tenukiOwner: boardState.tenukiOwner,
    })
    ...
    setBoardState({
      red: boardState.red,
      blue: sortPoints([...boardState.blue, [point[0], point[1]]]),
      toPlay: "red",
      tenukiOwner: boardState.tenukiOwner,
    })
  }
...
function renderBoard() {
  ...
  const hoverFill = turnRgbaText(nextColor, 0.12)
  const dragSourceKey =
    dragActive
      ? pointKey(state.drag.startPoint[0], state.drag.startPoint[1])
      : null
  const dragTargetKey = dragActive && state.drag.targetPoint ? pointKey(state.drag.targetPoint[0], state.drag.targetPoint[1]) : null
  ...
  for (const point of FIXED_BOARD_POINTS) {
    ...
    if (overlay) {
      fill = metricHeatFill(overlay.stoneFraction)
    ...
    if (overlay && !stone && dragTargetKey !== key) {
      appendText(hex.cx, hex.cy, percentText(overlay.stoneFraction))
    }
  }
  ...
  tenukiHoverHex.polygon.style.setProperty(
    "--hover-fill",
    turnRgbaText(boardState.toPlay, 0.12),
  )
  ...
  const tenukiHex = appendHex(TENUKI_POINT, {
    fill:
      tenukiMarked
        ...
        : (tenukiShaded ? metricHeatFill(state.tenukiEntry) : rgbText(OFF_WHITE_RGB)),
    className:
...
function cancelBoardInteractionForBoardChange() {
  boardPointerController?.cancel({ notify: false })
  state.drag = null
}

function finishPatternDrag(interaction, releasePoint) {
  const occupied = occupiedMap()
  const targetPoint = dragTargetFromPoint(interaction.startPoint, releasePoint, occupied)
  state.drag = null
  if (targetPoint) {
    setBoardState(rewriteDraggedStone(interaction.startPoint, targetPoint, interaction.dragData.sourceColor))
    commitBoardChange()
  } else {
    renderBoard()
  }
...
function cancelPatternPointerInteraction(interaction) {
...
boardPointerController = createBoardPointerController({
  ...
  pointsEqual,
  dragDataForPoint: patternDragData,
  onTap: activateBoardPoint,
  onDragStart: beginPatternDrag,
  onDragMove: movePatternDrag,
  onDrop: finishPatternDrag,
  onCancel: cancelPatternPointerInteraction,
})
...
async function loadRandomPattern({ maxMoves = null, minMoves = null } = {}) {
  await preloadAllPatternLayers()
  const layerIds = randomPatternLayerIds({ maxMoves, minMoves })
  await Promise.all(layerIds.map((layerId) => ensurePatternLayerLoaded(layerId)))
  const patterns = loadedPatternKeysForLayers(layerIds)
  if (patterns.length === 0) {
    elements.lookupStatus.textContent = layerIds.some((layerId) => patternLayerLoadFailed(layerId))
      ? "Pattern bundle failed to load."
      : "No precomputed patterns available."
    return
  }
  ...
  if (eligible.length === 0) {
    elements.lookupStatus.textContent = layerIds.some((layerId) => patternLayerLoadFailed(layerId))
      ? "Pattern bundle failed to load."
      : "No patterns available for that random bucket."
    return
  }
...
function randomPatternLayerIds({ maxMoves = null, minMoves = null } = {}) {
  const minMove = minMoves === null ? 1 : Number(minMoves)
  const maxMove = maxMoves === null ? 6 : Number(maxMoves)
  const layerIds = []
  if (minMove <= 5 && maxMove >= 1) {
    layerIds.push("base")
  }
  if (minMove <= 6 && maxMove >= 6) {
    layerIds.push("m6")
  }
  return layerIds.length > 0 ? layerIds : ["base"]
}

function loadedPatternKeysForLayers(layerIds) {
  const layerIdSet = new Set(layerIds.map((layerId) => String(layerId)))
  const seen = new Set()
  for (const row of state.dataLayers) {
    if (!layerIdSet.has(String(row.id))) {
      continue
    }
    for (const pattern of Object.keys(row?.data?.patterns || {})) {
      seen.add(pattern)
    }
  }
...
function manifestLayers() {
  ...
  return Array.isArray(layers)
    ? layers
      .filter((layer) => layer && typeof layer.bundle === "string" && typeof layer.id === "string")
      .map((layer) => ({ ...layer, id: String(layer.id) }))
    : []
}

async function ensurePatternManifestLoaded() {
  if (state.manifest) {
    return state.manifest
  }
  state.manifest = await fetchJson(MANIFEST_URL, { cache: "no-store" })
  return state.manifest
}

function patternLayerById(layerId) {
  return manifestLayers().find((layer) => String(layer.id) === String(layerId)) || null
}

async function ensurePatternLayerLoaded(layerId) {
  const layerIdText = String(layerId || "").trim()
  if (!layerIdText || isPatternLayerLoaded(layerIdText)) {
    return
  }
  const existingPromise = state.loadingPromiseByLayerId.get(layerIdText)
  if (existingPromise) {
    return existingPromise
  }
  state.errorByLayerId.delete(layerIdText)
  syncPatternLoadStatus()
  const promise = (async () => {
    await ensurePatternManifestLoaded()
    const layer = patternLayerById(layerIdText)
    if (!layer) {
      throw new Error(`Missing pattern layer: ${layerIdText}`)
    }
    const dataUrl = new URL(
      String(layer.bundle),
      new URL(MANIFEST_URL, window.location.href),
    ).toString()
    const data = normalizeLoadedData(await fetchArrayBuffer(dataUrl))
    if (!isPatternLayerLoaded(layerIdText)) {
      state.dataLayers.push({ id: layerIdText, data })
    }
    state.errorByLayerId.delete(layerIdText)
    state.dataError = null
  })().catch((error) => {
    state.errorByLayerId.set(layerIdText, String(error instanceof Error ? error.message : error))
  }).finally(() => {
    state.loadingPromiseByLayerId.delete(layerIdText)
    syncPatternLoadStatus()
    syncFromBoard()
  })
  state.loadingPromiseByLayerId.set(layerIdText, promise)
  return promise
}

async function preloadAllPatternLayers() {
  const manifest = await ensurePatternManifestLoaded().catch((error) => {
  ...
  if (manifest) {
    ...
    for (const layer of manifestLayers()) {
      void ensurePatternLayerLoaded(layer.id)
    }
  }
  syncFromBoard()
}
...
async function copyCanonicalPattern() {
  ...
  await copyButtonText(elements.canonicalPattern, text)
}
...
elements.randomLe4Btn.addEventListener("click", () => {
  randomBucketControls.set("le4")
})
elements.random5Btn.addEventListener("click", () => {
  randomBucketControls.set("5")
})
elements.random6Btn.addEventListener("click", () => {
  randomBucketControls.set("6")
})
