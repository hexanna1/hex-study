...
const FRACTION_MODE_ROW_COORD_BITPLANE = 3
...
function decodePresenceRows(bytes, patternCount, pairTable) {
  ...
  const flagByteLength = Math.ceil(patternCount / 8)
  const bitmapByteLength = Math.ceil(pairCount / 8)
  assertPatternBundleSize(bytes.length >= flagByteLength)
  ...
  const previous = new Uint8Array(pairCount)
  let offset = flagByteLength
  for (let entryIdx = 0; entryIdx < patternCount; entryIdx += 1) {
    const useBitmap = readPackedWordAtBit(view, 0, entryIdx, 1) === 1
    const current = new Uint8Array(previous)
    if (useBitmap) {
      assertPatternBundleSize(offset + bitmapByteLength <= bytes.length)
      for (let pairIdx = 0; pairIdx < pairCount; pairIdx += 1) {
        if (readPackedWordAtBit(view, offset, pairIdx, 1) === 1) {
          current[pairIdx] = current[pairIdx] === 1 ? 0 : 1
        }
      }
      offset += bitmapByteLength
    } else {
      const count = readUvarintAt(bytes, offset)
      offset = count.offset
      let pairIdx = -1
      for (let row = 0; row < count.value; row += 1) {
        const gap = readUvarintAt(bytes, offset)
        offset = gap.offset
        pairIdx += gap.value + 1
        assertPatternBundleSize(pairIdx >= 0 && pairIdx < pairCount)
        current[pairIdx] = current[pairIdx] === 1 ? 0 : 1
      }
    }
    for (let pairIdx = 0; pairIdx < pairCount; pairIdx += 1) {
      previous[pairIdx] = current[pairIdx]
      if (current[pairIdx] === 1) {
        coordRows[pairIdx].present[entryIdx] = 1
      }
    }
  }
...
function normalizeLoadedData(rawBuffer) {
  ...
  if (fractionMode !== FRACTION_MODE_ROW_COORD_BITPLANE) {
...
function parseFractionData(view, fractionOffset, keyStreamOffset, patternCount, pairCount, cellCount) {
  const rowMeanOffset = fractionOffset
  let offset = rowMeanOffset + Math.ceil((patternCount * PACKED_STONE_FRACTION_BITS) / 8)
...
function patternEntryForLookupInData(data, pattern) {
  ...
  const cells = []
  const rowMean = readBitplaneValue(
    view,
    data.fractionData.rowMeanOffset,
    data.pattern_count,
    entryIndex,
    PACKED_STONE_FRACTION_BITS,
  )
...
function beginInteraction(pointerId, point, stone, { tapOnly = false } = {}) {
  state.drag = {
    pointerId,
...
function renderBoard() {
  clearSvg()
  setupViewBox()
  const dragActive = Boolean(state.drag && state.drag.sourceColor !== null && !state.drag.tapOnly)
...
function handleBoardPointerDown(event) {
  ...
  const tapOnly = event.pointerType === "touch"
  beginInteraction(event.pointerId, point, stone, { tapOnly })
  if (stone && !tapOnly) {
    renderBoard()
...
function handleBoardPointerMove(event) {
  if (!state.drag || event.pointerId !== state.drag.pointerId || state.drag.sourceColor === null || state.drag.tapOnly) {
    return
  }
