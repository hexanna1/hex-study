...
const ENTRY_ROW_SIZE = 12
const CELL_ROW_SIZE = 4
...
function normalizeLoadedData(rawBuffer) {
  ...
  const keyBlobSize = view.getUint32(16, true)
  const entriesOffset = HEADER_SIZE
  const cellsOffset = entriesOffset + (patternCount * ENTRY_ROW_SIZE)
  const keyBlobOffset = cellsOffset + (cellCount * CELL_ROW_SIZE)
  const expectedSize = keyBlobOffset + keyBlobSize
  if (rawBuffer.byteLength !== expectedSize) {
    throw new Error("Pattern bundle size mismatch")
  }
  const keyBlob = new Uint8Array(rawBuffer, keyBlobOffset, keyBlobSize)
  const patterns = {}
  for (let idx = 0; idx < patternCount; idx += 1) {
    const offset = entriesOffset + (idx * ENTRY_ROW_SIZE)
    const keyOffset = view.getUint32(offset, true)
    const cellStart = view.getUint32(offset + 4, true)
    const keyLength = view.getUint8(offset + 8)
    const countFlags = view.getUint8(offset + 9)
    const tenukiStoneFraction = view.getUint16(offset + 10, true)
    const cellRowCount = countFlags & 0x3F
    const flags = countFlags & 0xC0
    if (keyOffset + keyLength > keyBlob.length) {
      throw new Error("Unsupported pattern key slice")
    }
    if (cellStart + cellRowCount > cellCount) {
      throw new Error("Unsupported pattern cell slice")
    }
    const pattern = textDecoder.decode(keyBlob.subarray(keyOffset, keyOffset + keyLength))
    const cells = []
    for (let cellIndex = cellStart; cellIndex < cellStart + cellRowCount; cellIndex += 1) {
      const cellOffset = cellsOffset + (cellIndex * CELL_ROW_SIZE)
      cells.push([
        view.getInt8(cellOffset),
        view.getInt8(cellOffset + 1),
        view.getUint16(cellOffset + 2, true),
      ])
    }
    const entry = {
      p: (flags & 0x40) !== 0 ? "red" : "blue",
      c: cells,
    }
    if ((flags & 0x80) !== 0) {
      entry.t = tenukiStoneFraction
    }
