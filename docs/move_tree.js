(() => {
  ...
  function rewriteMoveLinePreservingLegalTail({
    moves,
    cursor,
    moveIndex,
    targetMove,
    materializeLine = materializeBoardState,
  }) {
    const sourceMoves = Array.isArray(moves) ? moves : parseMoves(moves)
    const index = Number(moveIndex)
    const activeCursor = Number(cursor)
    if (
      !Number.isInteger(index)
      || !Number.isInteger(activeCursor)
      || index < 0
      || index >= activeCursor
      || index >= sourceMoves.length
      || typeof materializeLine !== "function"
    ) {
      return null
    }
    const normalizedTargetMove = formatLine([targetMove])
    if (!normalizedTargetMove || sourceMoves[index] === normalizedTargetMove) {
      return null
    }
    const nextMoves = [...sourceMoves]
    nextMoves[index] = normalizedTargetMove
    const keptMoves = []
    for (const move of nextMoves) {
      const candidate = [...keptMoves, move]
      try {
        materializeLine(candidate)
      } catch (_error) {
        break
      }
      keptMoves.push(move)
    }
    if (keptMoves.length <= index) {
      return null
    }
    return {
      moves: keptMoves,
      cursor: Math.min(activeCursor, keptMoves.length),
    }
  }
  ...
  function renderMoveTreeBoard({
    ...
    for (let row = 1; row <= boardSize; row += 1) {
      for (let col = 1; col <= boardSize; col += 1) {
        ...
        if (showCellCoords) {
          boardSvg.appendText(hex.cx, hex.cy, formatCell(col, row), "cell-text", stone?.textColor || null)
  ...
  window.HexMoveTree = {
    ...
    renderMoveTreeBoard,
    renderSideActionHex,
    rewriteMoveLinePreservingLegalTail,
    setHashFromLine,
    swapControlPoint,
    tryParseCell,
  }
