(() => {
  ...
  function renderBoard({
    ...
    const renderedBoard = renderMoveTreeBoard({
      ...
      boardState: board,
      cellOverlays: showPriors
        ? new Map([...analysisByKey].filter(([, row]) => row.visits > 0))
        : analysisByKey,
      isLegalCell: (col, row, currentBoard) => !currentBoard.occupied.has(pointKey(col, row)),
      mirrorRootCandidates: false,
      candidateFill: (row) => analysisOverlayFill(row, { mode: showPriors ? "prior" : "winrate" }),
      overlayPrimaryText: (row) => window.BoardAnalysis.overlayText(row, showPriors).primary,
      overlaySecondaryText: (row) => window.BoardAnalysis.overlayText(row, showPriors).secondary,
  ...
  analysis = window.BoardAnalysis.create(editor)
})()
