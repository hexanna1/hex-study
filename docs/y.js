(() => {
  ...
  function renderBoard({
    ...
    const showPriors = analysis?.showPriors() || false
    boardSvg.clear()
    ...
    for (const cell of cells) {
      ...
      const candidate = analysisByKey.get(key) || null
      const overlay = showPriors && !candidate?.visits ? null : candidate
      ...
      if (stone) {
        ...
        fill = analysisOverlayFill(overlay, { mode: showPriors ? "prior" : "winrate" }) || fill
      }
      ...
      if (display.showCoords) {
        ...
        const { primary, secondary } = window.BoardAnalysis.overlayText(overlay, showPriors)
    ...
    if (!dragActive) analysis?.renderPv(boardSvg, board.toPlay, display)
  }

  const editor = createBranchingBoardEditor({
    game: "y",
    elements,
    ...
    renderBoard,
    getBoardDecorations: () => analysis?.decorations() || {},
    shortcutHelpGroups: window.BoardAnalysis.shortcutHelpGroups,
  })
  analysis = window.BoardAnalysis.create(editor, "b10nbt-24")
})()
