...
const { CANDIDATE_HIGH, CANDIDATE_LOW, OFF_WHITE_RGB } = THEME
...
function decodeCompactNodes(rawNodes, view, moveStreamOffset, priorStreamOffset, deltaStreamOffset, childStreamOffset, exceptionOffset, boardSize, totalCandidateCount) {
  const nodes = []
  let nodeOffset = 0
  let candidateIndex = 0
  let moveBitOffset = 0
  let priorBitOffset = 0
  let deltaBitOffset = 0
  let childBitOffset = 0
  ...
  function decodeNode(line, parentIndex, move, parentEdgeRedMetric, ply) {
    ...
    const candidates = []
    const childLines = []
    let allCandidatesHaveChildren = false
    if (candidateCount > 0) {
      allCandidatesHaveChildren = readPackedWordAtBit(view, childStreamOffset, childBitOffset, 1) !== 0
      childBitOffset += 1
    }
    for (let idx = 0; idx < candidateCount; idx += 1) {
      const isSingleCandidateNode = candidateCount === 1
      const moveId = readPackedWordAtBit(view, moveStreamOffset, moveBitOffset, moveIdBits)
      moveBitOffset += moveIdBits
      if (!Number.isInteger(moveId)) {
        throw new Error("Unsupported opening candidate move")
      }
      const prior = readPackedWordAtBit(view, priorStreamOffset, priorBitOffset, PACKED_CANDIDATE_METRIC_BITS)
      priorBitOffset += PACKED_CANDIDATE_METRIC_BITS
  ...
  return { nodes, exceptionIndex, moveBitOffset, priorBitOffset, deltaBitOffset, childBitOffset }
}
...
function normalizeLoadedData(rawBuffer) {
  ...
  const moveIdBits = packedMoveIdBits(boardSize)
  const moveOffset = HEADER_SIZE + (nodeCount * NODE_ROW_SIZE)
  const rawNodes = []
  let offset = HEADER_SIZE
  let deltaBitLength = 0
  let childBitLength = 0
  ...
  const moveBitLength = candidateCount * moveIdBits
  const priorBitLength = candidateCount * PACKED_CANDIDATE_METRIC_BITS
  const priorOffset = moveOffset + Math.ceil(moveBitLength / 8)
  const deltaOffset = priorOffset + Math.ceil(priorBitLength / 8)
  const childOffset = deltaOffset + Math.ceil(deltaBitLength / 8)
  const exceptionOffset = childOffset + Math.ceil(childBitLength / 8)
  const decoded = decodeCompactNodes(rawNodes, view, moveOffset, priorOffset, deltaOffset, childOffset, exceptionOffset, boardSize, candidateCount)
  const usedSize = exceptionOffset + Math.ceil((decoded.exceptionIndex * PACKED_CANDIDATE_METRIC_BITS) / 8)
  if (
    decoded.moveBitOffset !== moveBitLength
    || decoded.priorBitOffset !== priorBitLength
    || decoded.deltaBitOffset !== deltaBitLength
    || decoded.childBitOffset !== childBitLength
...
async function copyCurrentLine() {
  await copyTextToClipboard(currentLineText())
}
