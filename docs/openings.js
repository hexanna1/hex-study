...
function decodeBundleParentIndex(rawParent, decodedNodesLength) {
  ...
  const rawParentValue = Number(rawParent)
  if (rawParentValue < 0) {
    throw new Error("Unsupported opening node parent index")
  }
  if (rawParentValue === 0) {
  ...
  const parentIndex = rawParentValue - 1
  if (parentIndex < -1) {
    throw new Error("Unsupported opening node parent index")
  }
...
function decodeCompactNode(rawNode, rawCandidates, decodedNodes, candidateStart, boardSize) {
  ...
  for (let idx = candidateStart; idx < candidateStart + candidateCount; idx += 1) {
    ...
    candidates.push({
      move: moveText,
      childLine: appendMoveToLine(line, moveText),
...
function normalizeLoadedData(rawBuffer) {
  ...
  for (let idx = 0; idx < nodeCount; idx += 1) {
    rawNodes.push({
      parentIndex: readUint24LE(view, offset),
      moveId: view.getInt16(offset + 3, true),
      isCore: (view.getUint8(offset + 5) & 0x80) !== 0,
      candidateCount: view.getUint8(offset + 5) & 0x7F,
    })
...
function renderBoard() {
  ...
  return renderMoveTreeBoard({
    ...
    candidateFill: (overlay) => {
      ...
      const denom = Math.log(Math.max(2, topChildSubtreeCount))
      let t = denom <= 0 ? 0 : (Math.log(Math.max(1, count)) / denom)
      t = clamp01(t) ** 1.1
      const wrBias = (winrate - 0.5) * 2.0
      t = clamp01(t + (0.35 * wrBias))
      return rgbText(lerpRgb(CANDIDATE_LOW, CANDIDATE_HIGH, t))
    },
