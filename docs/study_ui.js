(() => {
  ...
  function renderExternalLink(container, url, text) {
  ...
  function renderBoardLinks(container, hexWorldUrl) {
    if (!(container instanceof HTMLElement)) return
    container.replaceChildren()
    if (!hexWorldUrl) return
    const editorUrl = new URL("./hex.html", document.baseURI)
    editorUrl.hash = new URL(hexWorldUrl).hash
    renderExternalLink(container, editorUrl.href, "Hexata")
    container.append(document.createTextNode(" · "))
    renderExternalLink(container, hexWorldUrl, "HexWorld")
  }
  ...
  function isBoardShortcutSelect(event) {
    return event.target instanceof HTMLElement
      && event.target.matches("select[data-board-shortcuts]")
      && !["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Home", "End", "PageUp", "PageDown", "Escape", "Tab", "F4"].includes(event.key)
  }
  ...
  function handlePageButtonClick(event, url, action) {
  ...
  function handleStandardKeydown(event, actions) {
    ...
    if (event.shiftKey && String(event.key).toLowerCase() === "n" && actions.newGame) {
      event.preventDefault()
      if (!event.repeat) actions.newGame()
      return true
    }
    if ((event.key === "t" || event.key === "T") && actions.toggleOverlayMode) {
      event.preventDefault()
  ...
  function installHoldButton(button, action, isDisabled = null, { activateOnRelease = false } = {}) {
    ...
    button.addEventListener("pointerdown", (event) => {
      if (disabled() || event.button > 0
        || (activateOnRelease && event.pointerType === "touch" && event.isPrimary === false)) {
        return
      }
      event.preventDefault()
      stopHold()
      ...
      if (activateOnRelease) {
        scheduleRepeat(HOLD_START_DELAY_MS)
        return
      }
      if (!action() || disabled()) {
        stopHold()
    ...
    button.addEventListener("pointermove", (event) => {
      if (pointerActive && activateOnRelease && event.pointerId === activePointerId && !pointerInside(event)) {
        stopHold()
      }
    })
    button.addEventListener("pointerup", (event) => {
      if (pointerActive && activateOnRelease && event.pointerId === activePointerId
        && repeatCount === 0 && pointerInside(event) && !disabled()) action()
      stopHold()
    })
    ...
    window.addEventListener("pointerdown", (event) => {
      if (pointerActive && activateOnRelease && event.pointerType === "touch" && event.pointerId !== activePointerId) {
        stopHold()
      }
    })
  ...
  function installMoveNavigation({
    container,
    goFirst,
    goPrevious,
    goNext,
    goLast,
    setDisabled = setNavButtonDisabled,
  }) {
    const first = container.querySelector("#move-first-btn")
    const previous = container.querySelector("#move-prev-btn")
    const next = container.querySelector("#move-next-btn")
    const last = container.querySelector("#move-last-btn")
    for (const eventName of ["contextmenu", "selectstart"]) {
      container.addEventListener(eventName, (event) => {
        event.preventDefault()
      }, { capture: true })
    }
    for (const [button, navigate] of [[first, goFirst], [last, goLast]]) {
      button.addEventListener("click", () => {
        if (!navButtonDisabled(button)) {
          navigate()
        }
      })
    }
    // Step actions return whether the cursor moved so holds stop at boundaries.
    installHoldButton(previous, goPrevious)
    installHoldButton(next, goNext)
    return {
      sync({ atStart, atEnd }) {
        setDisabled(first, atStart)
        setDisabled(previous, atStart)
  ...
  function analysisOverlayFill(overlay, {
    ...
    if (mode === "prior") {
      ...
      if (prior === null) return emptyFill
      return overlay.isTopPrior ? rgbText(THEME.ANALYSIS_BEST_RGB) : metricHeatFill(prior, { lowRgb, highRgb })
    }
    ...
    if (overlay.isTopMove) return rgbText(THEME.ANALYSIS_BEST_RGB)
  ...
  function renderAnalysisPv(boardSvg, pv, toPlay, boardSize, labelForMove, firstMoveText) {
    if (!Array.isArray(pv) || pv.length < 2) return
    for (let i = 0; i < pv.length; i++) {
      const move = pv[i]
      const col = move % boardSize + 1
      const row = Math.floor(move / boardSize) + 1
      const rgb = (toPlay === "red") === (i % 2 === 0) ? THEME.RED_RGB : THEME.BLUE_RGB
      const hex = boardSvg.appendHex(col, row, {
        fill: rgbText(lerpRgb(rgb, THEME.OFF_WHITE_RGB, 0.5)),
        stroke: THEME.GRID_EDGE,
        strokeWidth: 1,
        className: "board-pv-ghost",
      })
      if (labelForMove) {
        boardSvg.appendText(hex.cx, hex.cy, labelForMove(col, row))
      } else if (i === 0) {
        const { primary, secondary } = firstMoveText
        if (primary && secondary) boardSvg.appendStackedText(hex.cx, hex.cy, primary, secondary)
        else if (primary || secondary) boardSvg.appendText(hex.cx, hex.cy, primary || secondary)
      } else {
        boardSvg.appendText(hex.cx, hex.cy, String(i + 1), "board-pv-label")
      }
    }
  }
