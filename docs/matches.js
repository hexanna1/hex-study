...
const BUNDLE_MAGIC = "HMB"
const BUNDLE_HEADER_SIZE = 7
...
const graphPreviewLabel = document.createElement("p")
const graphPreviewSvg = createSvgNode("svg")
elements.evalGraphTooltip.append(graphPreviewLabel, graphPreviewSvg)
let graphPreviewPoint = null
...
function parseHashState() {
  ...
  const decodedHash = hasHash ? decodeLocationHash() : ""
  if (decodedHash === null) {
    return {
      gameIndex: null,
      stepIndex: null,
  ...
  const raw = typeof decodedHash === "string" ? decodedHash.trim().toLowerCase() : ""
  if (!raw) {
    return {
      gameIndex: null,
      stepIndex: null,
...
function decodeMatchBundle(rawBuffer) {
  ...
  const gameCount = view.getUint32(3, true)
  const games = []
...
function renderMatchNav(game) {
  ...
  if (!game) {
    nav.hidden = true
    moveNavigation.sync({ atStart: true, atEnd: true })
  ...
  nav.hidden = false
  moveNavigation.sync({ atStart: currentIndex <= 0, atEnd: currentIndex >= lastStepIndex })
...
function renderExternalLinks(game) {
  ...
  container.replaceChildren()
  renderBoardLinks(container, currentHexWorldUrl(game))
...
function showEvalGraphTooltip(event, game, point) {
  if (event.pointerType === "touch") { hideEvalGraphTooltip(); return }
  graphPreviewLabel.textContent = evalGraphPointLabel(point)
  if (graphPreviewPoint !== point) {
    window.BoardGraphPreview.draw(graphPreviewSvg, {
      game: "hex", boardSize: game.board_size,
      board: materializeBoardState(parseMoves(point.line), game.board_size),
    })
    graphPreviewPoint = point
  }
  window.BoardGraphPreview.place(elements.evalGraphTooltip, event.clientX, event.clientY)
}
...
function renderEvalGraph(game) {
  ...
  for (const point of points) {
    ...
    hit.addEventListener("pointerenter", (event) => {
      showEvalGraphTooltip(event, game, point)
    })
    ...
    hit.addEventListener("pointercancel", hideEvalGraphTooltip)
...
window.addEventListener("blur", hideEvalGraphTooltip)
window.addEventListener("scroll", hideEvalGraphTooltip, true)
  ...
  const board = renderMoveTreeBoard({
    ...
    candidateFill: (overlay) => {
      ...
      return analysisHeatFill({
        weight: typeof overlay?.visits === "number" ? overlay.visits : 0,
        topWeight: topVisits,
        value: wr,
      })
    },
    ...
    onGoToLine: goToLine,
    onActivateLastStone: goPrevious,
...
function renderSideActionCandidateHex(node, board, topVisits) {
  ...
  const fill = state.overlayTextMode === "prior"
    ? (typeof candidate?.prior === "number" ? metricHeatFill(candidate.prior) : rgbText(OFF_WHITE_RGB))
    : (typeof wr === "number"
      ? analysisHeatFill({
        weight: typeof candidate?.visits === "number" ? candidate.visits : 0,
        topWeight: topVisits,
        value: wr,
      })
...
async function loadData() {
  ...
  state.loadingPromise = (async () => {
    ...
    if (typeof manifest?.bundle !== "string") {
    ...
    if (!parsed.valid) {
      replaceHash("")
      setCurrentGame(games[0].game_index, null, { updateHash: false })
      return { games, gamesByIndex }
    }
...
function syncFromLocationHash() {
  ...
  if (!parsed.valid) {
    replaceHash("")
    setCurrentGame(state.games[0].game_index, null, { updateHash: false })
    return
  }
