...
function decodeCompactNode(rawNode, family) {
  ...
  for (const row of rawNode?.c || []) {
    ...
    candidates.push({
      kind: "local",
      local,
      stone_fraction: decodeThousandths(row[2]),
    })
  ...
  if (Array.isArray(rawNode?.t) && rawNode.t.length >= 1) {
    candidates.push({
      kind: "tenuki",
      stone_fraction: decodeThousandths(rawNode.t[0]),
    })
    if (Number(rawNode.t[1] || 0) && entries.length > 0) {
      retained_lines.push(formatLine(family, [...entries, null]))
    }
  }
  return { line, retained_lines, candidates, importance: decodeThousandths(rawNode?.i) ?? 0 }
}
