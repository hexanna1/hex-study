...
const PACKED_NODE_TAIL_BITS = 24
...
const PACKED_NODE_TENUKI_CHILD_SHIFT = PACKED_NODE_TENUKI_PRESENT_SHIFT + 1
const PACKED_NODE_TENUKI_SF_SHIFT = PACKED_NODE_TENUKI_CHILD_SHIFT + 1
const HEX_SIZE = 26
...
function decodeCompactNode(rawNode, rawLocalRows, localChildBits, family, entries, cursors, nodes) {
  ...
  for (const childEntries of localChildren) {
    ...
    decodeCompactNode(cursors.rawNodes[cursors.node], rawLocalRows, localChildBits, family, childEntries, cursors, nodes)
  }
...
function normalizeLoadedData(rawBuffer) {
  ...
  const nodeTailOffset = HEADER_SIZE
  const localRowsOffset = nodeTailOffset + Math.ceil((nodeCount * PACKED_NODE_TAIL_BITS) / 8)
  const localChildBitsOffset = localRowsOffset + (localRowCount * LOCAL_ROW_SIZE)
  const expectedSize = localChildBitsOffset + Math.ceil(localRowCount / 8)
  ...
  for (let idx = 0; idx < nodeCount; idx += 1) {
    const word = readPackedWordAtBit(view, nodeTailOffset, idx * PACKED_NODE_TAIL_BITS, PACKED_NODE_TAIL_BITS)
    rawNodes.push({
      localCount: word & PACKED_NODE_LOCAL_COUNT_MASK,
  ...
  for (let idx = 0; idx < localRowCount; idx += 1) {
    const offset = localRowsOffset + (idx * LOCAL_ROW_SIZE)
    rawLocalRows.push({
      moveCode: view.getUint8(offset),
      stoneFraction: view.getUint16(offset + 1, true),
    })
  }
  const localChildBits = []
  for (let idx = 0; idx < localRowCount; idx += 1) {
    localChildBits.push(readPackedWordAtBit(view, localChildBitsOffset, idx, 1) !== 0)
  }
  const nodes = []
  const cursors = { node: 0, localRow: 0, nodeCount, rawNodes }
  if (nodeCount > 0) {
    decodeCompactNode(rawNodes[0], rawLocalRows, localChildBits, family, [], cursors, nodes)
  }
