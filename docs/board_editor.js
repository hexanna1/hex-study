(() => {
  ...
  function collectBoardEditorElements(statusId, root = document) {
    ...
    return {
      ...
      movePassBtn: elements["move-pass-btn"],
  ...
  const SHORTCUT_HELP_GROUPS = [
    {
      ...
      lines: [
        ...
        [
          { keys: ["←", "→"], text: "branch" },
          { keys: ["shift+←", "shift+→"], text: "move branch" },
        ],
      ],
    },
    {
  ...
  function renderShortcutHelpPopover(popover, groups = SHORTCUT_HELP_GROUPS) {
  ...
  function createBranchingBoardEditor({
    ...
    const variationListeners = new Set()
    const boardOrientationListeners = new Set()
    let notifiedBoardOrientation = ui.boardOrientation
    let variationContext = null

    function getVariation() {
      if (variationContext?.tree === session.tree
        && variationContext.cursor === session.tree.cursor
        && variationContext.boardSize === session.boardSize) return variationContext.value
      const nodes = [session.tree.root, ...currentPathNodes(session.tree)]
      let tail = nodes.at(-1)
      while (preferredChild(tail)) {
        tail = preferredChild(tail)
        nodes.push(tail)
      }
      const boardSize = session.boardSize
      const moves = nodes.slice(1).map((node) => node.move)
      const value = {
        cursor: currentPly(),
        points: nodes.map((node, ply) => ({
          ply,
          move: node.move,
          key: positionKey(game, boardSize, moves.slice(0, ply)),
          select: () => selectNode(node),
          preview: () => ({ game, boardSize, board: materializeCurrentBoardState(moves.slice(0, ply), boardSize) }),
        })),
      }
      variationContext = { tree: session.tree, cursor: session.tree.cursor, boardSize, value }
      return value
    }

    function subscribeVariation(listener) {
      variationListeners.add(listener)
      listener(getVariation())
      return () => variationListeners.delete(listener)
    }

    function subscribeBoardOrientation(listener) {
      boardOrientationListeners.add(listener)
      listener(ui.boardOrientation)
      return () => boardOrientationListeners.delete(listener)
    }
    ...
    function notifyPositionChanged(freshBoard = false) {
      for (const listener of positionListeners) {
        listener(currentPositionContext.position, { freshBoard })
      }
    }
    ...
    function commitTransition({
      ...
      const clearAnalysis = freshBoard || boardSize !== session.boardSize
      ...
      sync({ message, prepared, freshBoard: clearAnalysis })
      return true
    }
    ...
    function syncNavigationButtons() {
      ...
      moveNavigation.sync({
        atStart: session.tree.cursor === session.tree.root,
        atEnd: preferredChild(session.tree.cursor) === null,
      })
    ...
    function sync({
      ...
      if (positionChanged || freshBoard) {
        notifyPositionChanged(freshBoard)
      }
      refreshBoard()
      if (ui.boardOrientation !== notifiedBoardOrientation) {
        notifiedBoardOrientation = ui.boardOrientation
        for (const listener of boardOrientationListeners) listener(notifiedBoardOrientation)
      }
      if (variationListeners.size) {
        const previous = variationContext?.value
        const variation = getVariation()
        if (variation !== previous) {
          for (const listener of variationListeners) listener(variation)
        }
      }
    }
    ...
    function handleShortcutHelpShortcut(event) {
      ...
      if (target instanceof HTMLElement) {
        ...
        if (tag === "input" || tag === "textarea" || (tag === "select" && !isBoardShortcutSelect(event)) || target.isContentEditable) {
          return false
        }
      }
    ...
    function shiftCurrentBranch(direction) {
      return runTrackedEdit((tree) => {
    ...
    function handleBranchingNavigationKeydown(event) {
      ...
      if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
        ...
        if (event.shiftKey) shiftCurrentBranch(direction)
        else goSibling(direction)
        return true
      }
    ...
    renderShortcutHelpPopover(elements.shortcutHelpPopover, [...SHORTCUT_HELP_GROUPS, ...shortcutHelpGroups])
    ...
    installHoldButton(elements.sizePrevBtn, () => stepSize(-1), null, { activateOnRelease: true })
    installHoldButton(elements.sizeNextBtn, () => stepSize(1), null, { activateOnRelease: true })
    const moveNavigation = installMoveNavigation({
      container: elements.moveNav,
      goFirst,
      goPrevious: () => stepCursor(-1),
      goNext: () => stepCursor(1),
      goLast,
    })
    elements.movePassBtn.addEventListener("click", passMove)
    ...
    elements.sizeInput.addEventListener("change", applySizeInput)
    ...
    window.addEventListener("keydown", (event) => {
      ...
      if (!shouldIgnoreGlobalKeydown(event) && ["+", "=", "-", "_"].includes(event.key)) {
        event.preventDefault()
        if (!event.repeat) stepSize(event.key === "+" || event.key === "=" ? 1 : -1)
        return
      }
