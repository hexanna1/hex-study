...
def _neighbor_offsets(Δ_max: int) -> tuple[Point, ...]:
    if Δ_max < 0:
...
def _candidate_Δ_max_for_visible_stone_count(
    *,
    visible_stone_count: int,
    candidate_Δ_max: int,
    max_pair_delta: int,
) -> int:
    if int(visible_stone_count) == 1:
        return max(int(DEFAULT_CANDIDATE_Δ_MAX), int(max_pair_delta))
    return int(candidate_Δ_max)
...
def _generate_connected_geometries(*, total_stones: int, Δ_max: int) -> list[tuple[Point, ...]]:
    if total_stones <= 0:
        return []
    offsets = _neighbor_offsets(Δ_max)
    ...
    for geom in out:
        if not _is_connected(geom, Δ_max=Δ_max):
...
def enumerate_patterns(
    *,
    ...
    candidate_Δ_max: int = DEFAULT_CANDIDATE_Δ_MAX,
) -> dict[str, object]:
    ...
    for total in sorted({plus + minus for plus, minus in family_order}):
        geoms = _generate_connected_geometries(total_stones=total, Δ_max=int(max_pair_delta))
        ...
        for geom in geometries_by_total[total]:
            for plus_idx in combinations(range(total), plus_count):
                ...
                seen[pattern] = PatternRecord(
                    ...
                    candidate_Δ_max=_candidate_Δ_max_for_visible_stone_count(
...
def _default_artifact_dir(
    *,
    ...
    stem = (
        ...
        f"_d{int(candidate_Δ_max)}_span{int(max_pair_delta)}"
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--candidate-delta", type=int, default=DEFAULT_CANDIDATE_Δ_MAX)
...
def main() -> int:
    ...
    catalog = enumerate_patterns(
        ...
        candidate_Δ_max=int(args.candidate_delta),
    )
    out_dir = (
