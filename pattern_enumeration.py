...
from local_pattern_representative import placement_offset, serialize_position
...
def _is_connected(points: Iterable[Point], *, Δ_max: int) -> bool:
    ...
    while todo:
        ...
        for other in all_pts:
            if other in seen:
                continue
            if hex_distance_squared(cur, other) <= Δ_max:
                seen.add(other)
                todo.append(other)
...
def _max_pair_delta(points: Iterable[Point]) -> int:
    ...
    for i, a in enumerate(pts):
        for b in pts[i + 1 :]:
            best = max(best, hex_distance_squared(a, b))
    return int(best)
...
def _anchored_span_domain(Δ_max: int) -> tuple[Point, ...]:
    ...
    for q in range(-limit, limit + 1):
        for r in range(-limit, limit + 1):
            ...
            if point <= origin:
                continue
            if hex_distance_squared(origin, point) <= int(Δ_max):
                out.append(point)
    out.sort()
    return tuple(out)
