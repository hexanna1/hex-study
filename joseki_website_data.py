    ...
    for row in list(node.get("candidates") or []):
        ...
        if kind == "local":
            ...
            if child_line not in retained:
                continue
            local_rows.append([int(local[0]), int(local[1]), _encode_thousandths(stone_fraction)])
        elif kind == "tenuki":
            child_line = jn.format_single_track_line(
                family=family,
                entries=entries + (None,),
            ) if line else ""
            tenuki_row = [_encode_thousandths(stone_fraction), 1 if child_line in retained and line else 0]

    compact: dict[str, Any] = {
        "l": line,
        "c": local_rows,
        "i": _encode_thousandths(float(node.get("importance", 0.0))),
    }
