...
def _load_raw_nn_cache(path: Path) -> dict[str, dict[str, Any]]:
    ...
    for key, value in raw.items():
        ...
        out[key] = _decode_compact_raw_nn_payload(value)
    return out
...
def _save_raw_nn_cache(path: Path, cache: dict[str, dict[str, Any]]) -> None:
    ...
    tmp.write_text(
        json.dumps(
            {k: _encode_compact_raw_nn_payload(cache[k]) for k in sorted(cache)},
            ensure_ascii=True,
            separators=(",", ":"),
        ) + "\n",
        encoding="utf-8",
...
def _encode_millionths(value: Any) -> Any:
    if isinstance(value, bool):
        raise ValueError(f"raw-NN cache value is not numeric: {value!r}")
...
def _decode_millionths(value: Any) -> float:
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValueError(f"raw-NN cache value is not integer millionths: {value!r}")
...
def _decode_compact_raw_nn_payload(payload: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    if "r" in payload:
...
def _minimal_analyze_payload(payload: dict[str, Any], *, move_limit: int | None = None) -> dict[str, Any]:
    out: dict[str, Any] = {}
    root_eval = _payload_root_eval(payload)
    ...
    # Raw-NN CLI analyze emits moves in rank order; compact cache consumers
    # recover rank from list position and store only move plus prior.
    for row in _payload_moves(payload):
...
def _payload_analyze(payload: dict[str, Any]) -> dict[str, Any] | None:
...
def _payload_candidate(payload: dict[str, Any]) -> dict[str, Any] | None:
...
def _payload_root_eval(payload: dict[str, Any]) -> dict[str, Any] | None:
    analyze = _payload_analyze(payload)
    root_eval = analyze.get("root_eval") if isinstance(analyze, dict) else None
    if isinstance(root_eval, dict):
        return root_eval
    return None


def _payload_moves(payload: dict[str, Any]) -> list[dict[str, Any]]:
    container = _payload_analyze(payload)
    if not isinstance(container, dict):
        container = _payload_candidate(payload)
    moves = container.get("moves") if isinstance(container, dict) else None
    if isinstance(moves, list):
        return [row for row in moves if isinstance(row, dict)]
    return []
...
def _payload_to_play(payload: dict[str, Any], *, position_input: str | None = None) -> str:
    source = position_input if isinstance(position_input, str) and position_input.strip() else str(payload.get("hexworld") or "")
    if not source:
        return ""
    try:
        _size, _red, _blue, to_play = _position_state(source)
    except Exception:
        return ""
    return to_play
...
def _root_eval_side_to_play_winrate(payload: dict[str, Any]) -> float | None:
    ...
    if not isinstance(root_eval, dict):
        return None
    to_play = _payload_to_play(payload)
...
def _attach_stone_fractions(
    ...
    *,
    ...
    board_size_input = position_input if isinstance(position_input, str) and position_input.strip() else str(payload.get("hexworld") or "")
    board_size = _extract_board_size_from_input(board_size_input)
    to_play = _payload_to_play(payload, position_input=position_input)
    canonical_proxy = (
...
def _run_multi_position_analyze(
    *,
    ...
    for position, line in zip(positions, stdout_lines):
        ...
        if not isinstance(payload, dict) or not bool(payload.get("ok")) or str(payload.get("hexworld") or "").strip() != position:
            raise ValueError("Multi-position analyze returned a failed record")
        records[position] = payload
    return records


def _aggregate_moves(
    payload: dict[str, Any], *, position_input: str | None = None, allow_first_row_proxy_fallback: bool = False
) -> list[dict[str, Any]]:
    ...
    _attach_stone_fractions(
        payload,
        out,
        position_input=position_input,
        allow_first_row_proxy_fallback=allow_first_row_proxy_fallback,
    )
    return out
