...
static RuleContext rule_context(const Position &pos) {
  ...
  const std::vector<Point> &moving_stones =
      pos.red_to_play ? pos.red : pos.blue;
  auto add_equivalence = [&](Point loser, Point winner,
                             std::initializer_list<Point> required_empty) {
    for (Point point : required_empty)
      if (pos.occupied.count(key(point, pos.size)))
        return;
    out.canonical_moves[key(loser, pos.size)] = winner;
  };
  for (Point anchor : moving_stones) {
    int col = anchor.first, row = anchor.second, size = pos.size;
    if (pos.red_to_play && row == 3 && col <= size - 2)
      add_equivalence({col, 2}, {col + 1, 2},
                      {{col, 2}, {col + 1, 2}, {col, 1}, {col + 1, 1},
                       {col + 2, 1}});
    if (pos.red_to_play && row == size - 2 && col >= 3)
      add_equivalence({col, size - 1}, {col - 1, size - 1},
                      {{col, size - 1}, {col - 1, size - 1}, {col, size},
                       {col - 1, size}, {col - 2, size}});
    if (!pos.red_to_play && col == 3 && row <= size - 2)
      add_equivalence({2, row}, {2, row + 1},
                      {{2, row}, {2, row + 1}, {1, row}, {1, row + 1},
                       {1, row + 2}});
    if (!pos.red_to_play && col == size - 2 && row >= 3)
      add_equivalence({size - 1, row}, {size - 1, row - 1},
                      {{size - 1, row}, {size - 1, row - 1}, {size, row},
                       {size, row - 1}, {size, row - 2}});
  }
  return out;
}

static bool normalize_move(Point &move, const Position &pos,
                           const RuleContext &rules) {
  int move_key = key(move, pos.size);
  if (pos.occupied.count(move_key) || rules.dead.count(move_key))
    return false;
  auto mapped = rules.canonical_moves.find(move_key);
  for (int remaining = pos.size; mapped != rules.canonical_moves.end();
       remaining--) {
    if (remaining == 0)
      throw std::runtime_error("canonical move cycle");
    move = mapped->second;
    mapped = rules.canonical_moves.find(key(move, pos.size));
  }
  return true;
}
    ...
    if (config.size() == 2 && config[0] == "normalize-rules-v1") {
      ...
      while (std::getline(std::cin, line)) {
        ...
        for (const std::string &move_text : moves) {
          ...
          if (!normalize_move(move, pos, rules)) {
            std::cout << '-';
            continue;
          }
          std::cout << cell(move);
