...
int main() {
  try {
    ...
    if (config.size() != 10 || config[0] != "opening-v1")
    ...
    while (std::getline(std::cin, line)) {
      ...
      while (std::getline(policies, item, ';')) {
        ...
        cleaned++;
        bool keep = true;
        if (cleaned > top_k && prior < extra_prior_min) {
          double prior_log = -std::log10(std::max(1e-6, prior));
          double exponent = prior_log_step * prior_log +
                            rank_step * std::max(0, cleaned - top_k - 1) +
                            ply_step * std::max(0, ply - 1);
          double weight = std::pow(importance_min, exponent);
          keep = importance * ply_decay * weight >= importance_min;
        }
        if (!keep)
          continue;
