...
NODE_STRUCT = struct.Struct("<ihHH")
CANDIDATE_STRUCT = struct.Struct("<HBHH")
...
def _pack_optional_u16(value: int | None) -> int:
    if value is None:
        return NULL_U16
    if value < 0 or value >= NULL_U16:
        raise ValueError(f"bad u16 payload: {value!r}")
    return int(value)
...
def build_opening_bundle(*, artifacts_root: Path, board_size: int) -> bytes:
    ...
    for parent, move_id, importance, candidate_count in nodes:
        out.extend(NODE_STRUCT.pack(parent, move_id, importance, candidate_count))
    for move_id, retained, prior, mover_winrate in candidates:
        out.extend(
            CANDIDATE_STRUCT.pack(
                move_id,
                retained,
                _pack_optional_u16(prior),
                _pack_optional_u16(mover_winrate),
            )
        )
    return bytes(out)
