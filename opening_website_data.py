...
def _linearize_opening_graph(
    *,
    ...
    redirect_nodes: list[int] = []
    emitted_index_by_node: dict[int, int] = {}
    claimed_nodes = {root_node}

    def append_preorder(node_id: int) -> None:
        if node_id in emitted_index_by_node:
            raise ValueError(f"duplicate opening node: {node_id!r}")
        emitted_index_by_node[node_id] = len(nodes)
        node = graph[node_id]
        ...
        for candidate in node_candidates:
            ...
            if isinstance(target, int):
                if target in claimed_nodes:
                    is_redirect = True
                    redirect_nodes.append(target)
                else:
                    claimed_nodes.add(target)
                    tree_children.append(target)
            redirect_flags.append(is_redirect)
            candidates.append(
                {
                    "move_id": candidate["move_id"],
                    "prior": candidate["prior"],
    ...
    append_preorder(root_node)
    if len(emitted_index_by_node) != len(graph):
        raise ValueError("opening graph contains unreachable nodes")
    redirect_targets = [emitted_index_by_node[node_id] for node_id in redirect_nodes]
    return nodes, candidates, redirect_flags, redirect_targets
