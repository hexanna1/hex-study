...
def _pack_candidate_bitstream(
    *,
    ...
    child_bits: list[bool],
) -> bytes:
    move_rows: list[tuple[int, int]] = []
    prior_rows: list[tuple[int, int]] = []
    ...
    for move_id, prior, red_winrate, parent_edge_red_winrate, node_candidate_count in candidates:
        ...
        move_rows.append((move_id_i, move_id_bits))
        prior_rows.append((pack_optional_u10(prior), PACKED_CANDIDATE_METRIC_BITS))
    ...
    return (
        pack_little_endian_bits(move_rows, chunk_bytes=6)
        + pack_little_endian_bits(prior_rows, chunk_bytes=6)
        + pack_little_endian_bits(delta_rows, chunk_bytes=6)
        + pack_little_endian_bits(
            [(1 if has_child else 0, 1) for has_child in child_bits],
            chunk_bytes=6,
        )
        + pack_little_endian_bits(
...
def build_opening_bundle(*, artifacts_root: Path, board_size: int) -> bytes:
    ...
    def append_preorder(node_idx: int, parent_edge_red_winrate: int | None = None) -> None:
        ...
        if node_candidate_count:
            ...
            child_bits.append(child_count == node_candidate_count)
