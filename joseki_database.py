...
def _candidate_children_for_position(
    *,
    ...
    for forced_local in _forced_override_children(family=family, entries=entries):
...
def _prune_raw_nn_cache(*, family: str, output_path: Path) -> tuple[Path, Path, int, int]:
    ...
    for idx, node in enumerate(nodes):
        ...
        retained_lines = node.get(node_keys["retained_lines"])
        if not isinstance(retained_lines, list) or not all(
            isinstance(retained_line, str) for retained_line in retained_lines
        ):
            raise ValueError(f"joseki node {idx} requires retained lines")
        lines.append(line)
    if len(set(lines)) != len(lines) or "" not in lines:
        raise ValueError("joseki artifact requires unique lines and one root")

    by_line = dict(zip(lines, nodes))
    ...
    for node in nodes:
        ...
        for retained_line_value in node.get(node_keys["retained_lines"], []) or []:
...
def _prepare_record_or_state(
    ...
    *,
    ...
    root_payload: dict[str, Any] | None,
    realized: RealizedPosition,
) -> tuple[dict[str, Any], PreparedExpansion | None]:
    position = realized[0]
...
def _finalize_node_expansion(
    ...
    *,
    ...
    if top_k is not None and len(ordinary_retained_candidates) > top_k:
        ...
        ordinary_retained_candidates = forced_candidates + ordinary_candidates[:top_k]
        ordinary_retained_candidates.sort(key=lambda item: (-item[1], item[2]))
        ...
        while frontier:
            ...
            realized_positions = [
                _realize_position(
                    family=node.family,
                    board_size=board_size,
                    realized_moves=node.realized_moves,
                )
                for node in frontier
            ]
            positions = [realized[0] for realized in realized_positions]
            ...
            prepared_selections: list[tuple[JosekiNode, PreparedExpansion] | None] = []
            immediate_records: list[dict[str, Any]] = []
            for node, position, realized in zip(frontier, positions, realized_positions):
                ...
                if prepared is None:
                    ...
                    prepared_selections.append(None)
                    continue
            ...
            prepared_items: list[tuple[JosekiNode, NodeExpansion] | None] = []
