...
def _empty_raw_nn_cache() -> dict[str, dict[str, Any]]:
    return {"values": {}, "analyze": {}}
...
def _load_raw_nn_cache(path: Path) -> dict[str, dict[str, Any]]:
    ...
    if not isinstance(raw_values, dict) or not isinstance(raw_analyze, dict):
        raise ValueError(f"Raw-NN cache must contain values/analyze tables: {path}")
    values: dict[str, float] = {}
    for key, value in raw_values.items():
        if not isinstance(key, str) or isinstance(value, bool) or not isinstance(value, int):
            raise ValueError(f"Bad raw-NN value cache entry in {path}: {key!r}")
        values[key] = lps._decode_millionths(value)
    analyze: dict[str, dict[str, Any]] = {}
    for key, value in raw_analyze.items():
        if not isinstance(key, str) or not isinstance(value, dict):
            raise ValueError(f"Bad raw-NN analyze cache entry in {path}: {key!r}")
        ...
        analyze[key] = decoded
    out: dict[str, dict[str, Any]] = {"values": values, "analyze": analyze}
    return out


def _encode_raw_nn_cache(cache: dict[str, dict[str, Any]]) -> dict[str, Any]:
    values = cache.get("values")
    analyze = cache.get("analyze")
    if not isinstance(values, dict) or not isinstance(analyze, dict):
        raise ValueError("raw-NN cache missing values/analyze tables")
    return {
        "values": {
            k: lps._encode_millionths(values[k])
            for k in sorted(values)
        },
        "analyze": {
            k: lps._encode_compact_raw_nn_payload(analyze[k])
            for k in sorted(analyze)
        },
    }
...
def _save_raw_nn_cache(path: Path, cache: dict[str, dict[str, Any]]) -> None:
    ...
    with lock_path.open("a", encoding="utf-8") as lockf:
        ...
        merged["values"].update({k: float(v) for k, v in cache["values"].items()})
        merged["analyze"].update({k: dict(v) for k, v in cache["analyze"].items()})
        tmp = path.with_name(path.name + ".tmp")
        tmp.write_text(
            json.dumps(
...
def _raw_nn_values(cache: dict[str, dict[str, Any]]) -> dict[str, float]:
    values = cache.get("values")
    if not isinstance(values, dict):
        raise ValueError("raw-NN cache missing values table")
    return values


def _raw_nn_analyze(cache: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    analyze = cache.get("analyze")
    if not isinstance(analyze, dict):
        raise ValueError("raw-NN cache missing analyze table")
    return analyze
...
def main() -> int:
    ...
    def compact_analyze_payloads(
        *,
        ...
        for position in [str(p).strip() for p in position_inputs if str(p).strip()]:
            ...
            if lps._is_valid_compact_raw_nn_payload(cached):
        ...
        if missing_positions:
            ...
            for position in missing_positions:
                ...
                analyze_cache[lps._cache_key(position)] = reduced
    ...
    def candidate_rows_from_cache(
        *,
        ...
        value_cache = _raw_nn_values(raw_nn_cache)
        for move in moves:
            cached = value_cache.get(lps._cache_key(position_input, move))
    ...
    def batch_fill_candidate_cache(
        ...
        for request_id, position_input, moves in request_specs:
            for move in moves:
                ...
                cached = value_cache.get(child_key)
        ...
        for start in range(0, len(key_items), 250):
            ...
            for child_key, child_position in chunk:
                ...
                value_cache[child_key] = float(red_wr)
                raw_nn_cache_dirty = True
        return total_cache_miss_moves
