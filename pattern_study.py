...
def _select_root_tenuki_move(
    ...
    *,
    ...
    moves = _compact_analyze_moves(payload)
    if moves:
...
def _empty_raw_nn_cache() -> dict[str, Any]:
    return {}


def _load_raw_nn_cache(path: Path) -> dict[str, Any]:
    if not path.exists():
        return _empty_raw_nn_cache()
    ...
    for key, value in raw.items():
        ...
        if isinstance(value, dict):
            ...
            if not lps._is_valid_compact_raw_nn_payload(decoded):
                raise ValueError(f"Bad raw-NN analyze payload in {path}: {key!r}")
            cache[key] = decoded
            continue
...
def _raw_nn_analyze(cache: dict[str, Any]) -> dict[str, Any]:
    return cache


def _compact_analyze_moves(payload: dict[str, Any]) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    def compact_analyze_payloads(
        *,
        ...
        t0 = time.time()
        analyze_cache = _raw_nn_analyze(raw_nn_cache)
        ...
        for position in [str(p).strip() for p in position_inputs if str(p).strip()]:
            ...
            cached = analyze_cache.get(key) if key in persistent_keys else None
        ...
        if missing_positions:
            fetched = lps._run_multi_position_analyze(
                hexata_main=hexata_main,
                position_inputs=missing_positions,
            )
            for position in missing_positions:
                payload = fetched.get(position)
                if not isinstance(payload, dict):
                    raise ValueError(f"Missing raw-NN analyze payload for {position!r}")
                reduced = lps._minimal_analyze_payload(payload)
                if lps._cached_payload_red_winrate(reduced) is None:
                    raise ValueError(f"Raw-NN analyze payload missing root winrate for {position!r}")
                key = lps._cache_key(position)
                if key in persistent_keys:
                    analyze_cache[key] = reduced
    ...
    def batch_fill_candidate_cache(
        ...
        for start in range(0, len(key_items), 250):
            ...
            for child_key, child_position in chunk:
                ...
                root_eval = lps._payload_root_eval(payload) if isinstance(payload, dict) else None
                ...
                if child_key in persistent_child_keys:
                    analyze_cache[child_key] = float(red_wr)
                    raw_nn_cache_dirty = True
        return total_cache_miss_moves
    ...
    if extracted.to_play_at_cursor == "blue":
        ...
        try:
            canon_payloads, _canon_cache_hits, elapsed_canon = compact_analyze_payloads(
        ...
        _write_raw_payload(
            ...
            command=["python3", str(hexata_main), "cli", "analyze", "-"],
            ok=True,
            err="",
            payload={"source": "raw_nn_cache_analyze", "hexworld": canonical_blue_probe_position, "payload": payload_canon},
        )
        moves = _compact_analyze_moves(payload_canon)
        if not moves:
    ...
    try:
        root_payload_by_position, probe_reused_roots, _probe_elapsed = compact_analyze_payloads(
