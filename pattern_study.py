...
def _select_root_tenuki_move(
    ...
    *,
    ...
    moves = lps._payload_moves(payload)
    if moves:
...
def _load_raw_nn_cache(path: Path) -> dict[str, float]:
    ...
    for key, value in raw.items():
        ...
        out[key] = float(value) / 1_000_000.0
    return out


def _encode_cache_winrate(value: float) -> int:
...
def _save_raw_nn_cache(path: Path, cache: dict[str, float]) -> None:
    ...
    if fcntl is None:
        ...
        tmp.write_text(
            json.dumps(
                {k: _encode_cache_winrate(cache[k]) for k in sorted(cache)},
                ensure_ascii=True,
                separators=(",", ":"),
            ) + "\n",
            encoding="utf-8",
    ...
    with lock_path.open("a", encoding="utf-8") as lockf:
        ...
        tmp.write_text(
            json.dumps(
                {k: _encode_cache_winrate(merged[k]) for k in sorted(merged)},
                ensure_ascii=True,
                separators=(",", ":"),
            ) + "\n",
            encoding="utf-8",
...
def main() -> int:
    ...
    if extracted.to_play_at_cursor == "blue":
        ...
        if not ok_canon:
            return fail()
        moves = lps._payload_moves(payload_canon)
        if not moves:
