...
def _default_debug_dir_name(
    *,
    ...
    return f"{stamp}-s{int(board_size)}-d{int(candidate_Δ_max)}-{movelist_slug}"
...
def _select_root_tenuki_move(
    ...
    *,
    ...
    moves = _compact_raw_nn_moves(payload)
    if moves:
        ...
        for i, entry in enumerate(moves):
            ...
            if pattern_cells_abs:
                min_Δ = min(hex_distance_squared((col, row), p) for p in pattern_cells_abs)
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--symmetry",
        choices=["identity", "rotations", "d6"],
        default=None,
        help="Representative symmetry policy (default: d6)",
    )
    ap.add_argument(
        "--tenuki-delta-min",
        dest="tenuki_Δ_min",
...
def _load_raw_nn_cache(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {}
    ...
    for key, value in raw.items():
        if not isinstance(key, str):
            raise ValueError(f"Bad raw-NN cache key in {path}: {key!r}")
        if isinstance(value, int) and not isinstance(value, bool):
            cache[key] = lps._decode_millionths(value)
            continue
        if isinstance(value, dict):
            decoded = lps._decode_compact_raw_nn_payload(value)
            if not lps._is_valid_compact_raw_nn_payload(decoded):
                raise ValueError(f"Bad raw-NN payload in {path}: {key!r}")
            cache[key] = decoded
            continue
...
def _encode_raw_nn_cache(cache: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for key in sorted(cache):
        value = cache[key]
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            out[key] = lps._encode_millionths(float(value))
        elif isinstance(value, dict):
            out[key] = lps._encode_compact_raw_nn_payload(value)
        else:
            raise ValueError(f"Bad raw-NN cache payload: {key!r}")
    return out


def _save_raw_nn_cache(path: Path, cache: dict[str, Any]) -> None:
    ...
    with lock_path.open("a", encoding="utf-8") as lockf:
        ...
        for key, value in cache.items():
            ...
            if isinstance(existing, dict) and isinstance(value, (int, float)) and not isinstance(value, bool):
                continue
            merged[key] = value
        tmp = path.with_name(path.name + ".tmp")
        tmp.write_text(
            json.dumps(
                _encode_raw_nn_cache(merged),
                ensure_ascii=True,
                separators=(",", ":"),
            ) + "\n",
            encoding="utf-8",
...
def _compact_raw_nn_moves(payload: dict[str, Any]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for i, row in enumerate(lps._cached_payload_moves(payload)):
        parsed = lps._cached_payload_move_prior(row)
        if parsed is None:
            continue
        move, prior = parsed
        item: dict[str, Any] = {"move": move, "rank": i + 1}
        if isinstance(prior, (int, float)):
            item["prior"] = float(prior)
        out.append(item)
    return out
...
def _apply_ablation_calibration_to_summary_rows(
    *,
    ...
    for exp, rows in by_experiment.items():
        ...
        best_move = str(best_entry.get("move") or "").strip().lower()
...
def main() -> int:
    ...
    symmetry_policy = str(args.symmetry or "d6")
    ...
    movelist_slug = sout.movelist_slug_from_hexworld(args.hexworld)
    ...
    candidate_value_cache: dict[str, float] = {}
    raw_nn_cache_dirty = False
    def _finish(code: int) -> int:
        if raw_nn_cache_dirty:
    ...
    def compact_raw_nn_payloads(
        *,
        position_inputs: list[str],
        persistent_keys: set[str],
    ) -> tuple[dict[str, dict[str, Any]], int, float]:
        nonlocal raw_nn_cache_dirty
        t0 = time.time()
        payloads: dict[str, dict[str, Any]] = {}
        missing_positions: list[str] = []
        seen_missing: set[str] = set()
        cache_hits = 0
        for position in [str(p).strip() for p in position_inputs if str(p).strip()]:
            key = lps._cache_key(position)
            cached = raw_nn_cache.get(key) if key in persistent_keys else None
            if key in persistent_keys and lps._is_valid_compact_raw_nn_payload(cached):
        ...
        if missing_positions:
            fetched = lps._run_multi_position_raw_nn_native(
                position_inputs=missing_positions,
                board_size=board_size,
                move_limit=(board_size * board_size) + 1,
            )
            for position in missing_positions:
                reduced = fetched.get(position)
                if not isinstance(reduced, dict):
                    raise ValueError(f"Missing raw-NN payload for {position!r}")
                if lps._cached_payload_red_winrate(reduced) is None:
                    raise ValueError(f"Raw-NN payload missing root winrate for {position!r}")
                key = lps._cache_key(position)
                if key in persistent_keys:
                    raw_nn_cache[key] = reduced
                    raw_nn_cache_dirty = True
                payloads[position] = reduced
        return payloads, cache_hits, time.time() - t0
    ...
    def candidate_rows_from_cache(
        *,
        ...
        for move in moves:
            cached = candidate_value_cache.get(lps._cache_key(position_input, move))
        ...
        debug_payload = {
            "source": "raw_nn_cache_batchrawnn",
            "hexworld": position_input,
            "rows": rows,
        }
    ...
    def batch_fill_candidate_cache(
        ...
        persistent_parent_positions: set[str],
    ) -> int:
        nonlocal raw_nn_cache_dirty
        child_position_by_key: dict[str, str] = {}
        persistent_child_keys: set[str] = set()
        total_cache_miss_moves = 0
        for _request_id, position_input, moves in request_specs:
            persist_children = lps._cache_key(position_input) in persistent_parent_positions
            for move in moves:
                child_key = lps._cache_key(position_input, move)
                if persist_children:
                    persistent_child_keys.add(child_key)
                cached = (
                    raw_nn_cache.get(child_key)
                    if persist_children
                    else candidate_value_cache.get(child_key)
                )
                if persist_children and lps._is_valid_compact_raw_nn_payload(cached):
                    red_wr = lps._cached_payload_red_winrate(cached)
                    if isinstance(red_wr, (int, float)):
                        candidate_value_cache[child_key] = float(red_wr)
                        continue
                if isinstance(cached, (int, float)):
                    candidate_value_cache[child_key] = float(cached)
                    continue
                child_position = lps._position_after_move(position_input, move)
                if child_key not in child_position_by_key:
                    total_cache_miss_moves += 1
                    child_position_by_key[child_key] = child_position
        if not child_position_by_key:
            return total_cache_miss_moves
        key_items = list(child_position_by_key.items())
        for start in range(0, len(key_items), RAW_NN_BATCH_CHUNK_SIZE):
            chunk = key_items[start : start + RAW_NN_BATCH_CHUNK_SIZE]
            payload_by_position = lps._run_multi_position_raw_nn_native(
            ...
            for child_key, child_position in chunk:
                ...
                if not isinstance(red_wr, (int, float)):
                    raise ValueError(f"Raw-NN payload missing root winrate for {child_position!r}")
                candidate_value_cache[child_key] = float(red_wr)
                if child_key in persistent_child_keys:
    ...
    if extracted.to_play_at_cursor == "blue":
        ...
        canonical_blue_probe_position = serialize_position(
            board_size=int(board_size),
        ...
        try:
            canon_payloads, _canon_cache_hits, elapsed_canon = compact_raw_nn_payloads(
                position_inputs=[canonical_blue_probe_position],
                persistent_keys={lps._cache_key(canonical_blue_probe_position)},
            )
        except Exception as exc:
            lps._log(f"[canonical blue baseline] failed: {exc}")
            return fail()
        payload_canon = canon_payloads.get(canonical_blue_probe_position)
        if not isinstance(payload_canon, dict):
            raise ValueError("Canonical move3 probe failed: missing cached payload")
        _write_raw_payload(
            exp_dir=raw_dir,
            filename="canonical_blue_probe.json",
            experiment_name="canonical_blue_probe",
            command=lps._native_batch_raw_nn_command(
                board_size=board_size,
                move_limit=(board_size * board_size) + 1,
            ),
            ok=True,
            err="",
            payload={"source": "raw_nn_cache_batchrawnn", "hexworld": canonical_blue_probe_position, "payload": payload_canon},
        )
        moves = _compact_raw_nn_moves(payload_canon)
        if not moves:
        ...
        for move_i, entry in enumerate(moves):
            ...
            try:
                ...
                continue
            rank_val = entry.get("rank")
            rank = int(rank_val) if isinstance(rank_val, (int, float)) else (move_i + 1)
            prior_val = entry.get("prior")
            prior = float(prior_val) if isinstance(prior_val, (int, float)) else float("-inf")
            ranked_move3.append((-prior, rank, move_i, mv))
        if not ranked_move3:
        ...
        shared_without_position = serialize_position(
            board_size=int(board_size),
    ...
    positions_to_evaluate: list[str] = []
    ...
    for exp, _rep, without_position in probe_rows:
        ...
        if with_key not in seen_probe_keys:
            ...
            positions_to_evaluate.append(with_position)
        without_key = lps._cache_key(without_position)
        if without_key not in seen_probe_keys:
            seen_probe_keys.add(without_key)
            positions_to_evaluate.append(without_position)

    try:
        root_payload_by_position, probe_reused_roots, _probe_elapsed = compact_raw_nn_payloads(
            position_inputs=positions_to_evaluate,
            persistent_keys={lps._cache_key(shared_without_position)},
        )
    except Exception as exc:
    ...
    for i, (exp, rep, without_position) in enumerate(probe_rows, start=1):
        ...
        without_payload = root_payload_by_position.get(without_position)
        root_red_wr_without = lps._cached_payload_red_winrate(without_payload) if isinstance(without_payload, dict) else None
        ...
        root_red_wr_with = lps._cached_payload_red_winrate(payload)
        if not isinstance(root_red_wr_with, (int, float)):
            raise ValueError(f"Missing cached root probe winrate for {position}")
        root_ablation_with_wr = float(root_red_wr_with) if extracted.to_play_at_cursor == "red" else (1.0 - float(root_red_wr_with))
    ...
    batch_raw_nn_cmd = lps._native_batch_raw_nn_command(board_size=board_size, move_limit=1)
    ...
    try:
        total_cache_miss_moves = batch_fill_candidate_cache(
            candidate_request_specs,
            persistent_parent_positions={lps._cache_key(shared_without_position)},
        )
    except Exception as exc:
        batch_err = str(exc)
            ...
            manifest["ablation_calibration"] = sout._redact_personal_obj(
                {
                    ...
                    "root_eval_mode": "batchrawnn/full_policy",
                    "candidate_eval_mode": "batchrawnn/child_positions",
