...
def movelist_slug_from_hexworld(hexworld: str) -> str:
...
def _parse_local_candidate_key(key: str) -> tuple[int, int] | None:
    try:
    ...
    *,
    ...
    for prow in pooled_rows:
        ...
        if key == "tenuki":
            ...
            continue
        rel = _parse_local_candidate_key(key)
        if rel is None:
            continue
        ...
        local_cells.append({**base, "local_rel": [int(rel_can[0]), int(rel_can[1])]})

    basis_points = list(pattern_plus_rel) + list(pattern_minus_rel) + [
        (int(cell["local_rel"][0]), int(cell["local_rel"][1])) for cell in local_cells
    ]
    if not basis_points:
        return None
    cells = sorted(
        local_cells + tenuki_cells,
...
def _write_local_map_spec_json(path: Path, spec: dict[str, Any]) -> None:
    aj.dump_pattern_tile(path, spec)


def _load_local_map_spec_json(path: Path) -> dict[str, Any]:
    raw = aj.load_pattern_tile(path)
    tile_keys = aj.PATTERN_TILE_KEYS
    cell_keys = aj.PATTERN_CELL_KEYS
    pattern = str(raw.get(tile_keys["pattern"]) or "")
    parsed = parse_pattern(pattern)
    if not isinstance(parsed, LabeledPattern):
        raise ValueError(f"bad pattern tile: {path}")
    cells: list[dict[str, Any]] = []
    for rank, row in enumerate(raw.get(tile_keys["cells"]) or [], start=1):
        local = row.get(cell_keys["local_rel"])
        cell: dict[str, Any] = {
            "kind": "local" if local is not None else "tenuki",
            "stone_fraction": float(row[cell_keys["stone_fraction"]]),
            "rank": rank,
        }
        if local is not None:
            cell["local_rel"] = [int(local[0]), int(local[1])]
        cells.append(cell)
    return {
        "pattern": pattern,
        "to_play": "red" if len(parsed.minus) - len(parsed.plus) <= 0 else "blue",
        "cells": cells,
    }
...
def _draw_local_map_spec(
    ...
    *,
    ...
    basis_points = list(pattern_red_rel) + list(pattern_blue_rel) + [tuple(r["local_rel"]) for r in local_candidates]
    if not basis_points:
        return False
...
def _write_local_pooled_map_artifacts(
    *,
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    pattern_plus_rel, pattern_minus_rel = _pattern_points_from_spec(str(spec["pattern"]))
    local_candidates = [
        cell
        for cell in list(spec.get("cells") or [])
        if isinstance(cell, dict) and str(cell.get("kind") or "") == "local" and isinstance(cell.get("local_rel"), list)
    ...
    board_size, _mapped = _minimal_square_board_layout(
        list(pattern_plus_rel) + list(pattern_minus_rel) + [
            tuple(int(x) for x in cell["local_rel"]) for cell in local_candidates
        ]
    )
