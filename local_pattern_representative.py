...
from hex_symmetry import ROTATIONS, D6, apply_transform_ax, hex_distance_squared
...
def _ensure_hexata_imports() -> None:
    hexata = str((Path(__file__).resolve().parent / ".." / "hexata").resolve())
...
from board import (  # type: ignore
    Board as BOARD_CLS,
    MAX_BOARD_SIZE as _HEXATA_MAX_BOARD_SIZE,
    MIN_BOARD_SIZE as _HEXATA_MIN_BOARD_SIZE,
    Side as SIDE_ENUM,
    coord_to_human as COORD_TO_HUMAN,
)
from formats.hexworld import (  # type: ignore
    cell_to_col_row as CELL_TO_COL_ROW,
    parse_hexworld_position as PARSE_HEXWORLD_POSITION,
)

MIN_BOARD_SIZE = int(_HEXATA_MIN_BOARD_SIZE)
MAX_BOARD_SIZE = int(_HEXATA_MAX_BOARD_SIZE)
del _HEXATA_MIN_BOARD_SIZE, _HEXATA_MAX_BOARD_SIZE
...
def _transform_ids_for_policy(symmetry: str) -> list[int]:
    ...
    raise ValueError("symmetry must be one of: identity, rotations, d6")
...
def select_available_pass_proxy_cell(*, board_size: int, to_play: str, occupied: set[Point]) -> Point:
...
def serialize_position(
    *,
    ...
    past_stream = serialize_position_stream(
        red_cells=red_cells,
...
def build_candidates_for_representative(
    *,
    ...
    proxy_cell = point_to_cell(
        *select_available_pass_proxy_cell(
            board_size=board_size,
            to_play=rep.to_play_at_cursor,
            occupied=occupied,
        )
    )
    ...
    for row in range(1, board_size + 1):
        for col in range(1, board_size + 1):
            ...
            if p in occupied:
                continue
            if min(hex_distance_squared(p, q) for q in pattern_cells) <= candidate_Δ_max:
                cand.append(p)
    ...
    *,
    ...
    for i, rep in enumerate(representatives, start=1):
        ...
        exp_meta[name] = {
            ...
            "board_offset": [int(rep.board_offset[0]), int(rep.board_offset[1])],
