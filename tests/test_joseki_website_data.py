...
class JosekiWebsiteDataTests(unittest.TestCase):
    def _decode_bundle(self, payload: bytes) -> dict[str, object]:
        ...
        for _ in range(node_count):
            word = int.from_bytes(payload[offset:offset + jwd.NODE_ROW_SIZE], "little")
            parent_plus_1 = word & ((1 << jwd.PACKED_NODE_PARENT_BITS) - 1)
            move_code = (word >> jwd.PACKED_NODE_PARENT_BITS) & ((1 << jwd.PACKED_NODE_MOVE_BITS) - 1)
            local_count = (
                word >> (jwd.PACKED_NODE_PARENT_BITS + jwd.PACKED_NODE_MOVE_BITS)
            ) & ((1 << jwd.PACKED_NODE_LOCAL_COUNT_BITS) - 1)
    ...
    def test_build_family_bundle_compacts_parent_linked_nodes(self) -> None:
        ...
        self.assertEqual(
            decoded["nodes"],
            [
                [0, 0, 1, True, False, False, 0],
                [1, 54, 1, True, True, True, 1000],
                [2, 255, 0, False, False, False, 0],
            ],
        )
    ...
    def test_write_joseki_bundles_writes_manifest_and_hashed_files(self) -> None:
        ...
        self.assertEqual(bundle_a["nodes"], [[0, 0, 0, True, False, False, 0]])
        self.assertEqual(bundle_o["nodes"], [[0, 0, 0, True, False, False, 0]])
