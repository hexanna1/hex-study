...
class PatternStudyBatchTests(unittest.TestCase):
    ...
    def test_default_out_dir_uses_catalog_parent(self):
        path = Path("artifacts/interior_patterns_m4_d7_span12/catalog.json")
        out_dir = pattern_study_batch._default_out_dir(catalog_path=path)
        self.assertEqual(out_dir, Path("artifacts") / "interior_patterns_m4_d7_span12")
    ...
    def test_load_catalog_targets_uses_hexworld_and_catalog_delta(self):
        ...
        self.assertEqual(
            targets,
            (
                pattern_study_batch.StudyTarget(delta=7, hexworld="https://hexworld.org/board/#21c1,k11"),
                pattern_study_batch.StudyTarget(delta=5, hexworld="https://hexworld.org/board/#21c1,k10k11"),
            ),
        )

    def test_target_json_path_uses_stable_slug_only(self):
        out_dir = Path("/tmp/example-batch")
        target = pattern_study_batch.StudyTarget(delta=12, hexworld="https://hexworld.org/board/#21c1,k11:p")
    ...
    def test_run_one_reuses_existing_valid_tile_json(self):
        with TemporaryDirectory() as td:
            ...
            target = pattern_study_batch.StudyTarget(delta=3, hexworld="https://hexworld.org/board/#21c1,k11")
    ...
    def test_build_contact_sheet_items_preserves_index_order(self):
        with TemporaryDirectory() as td:
            ...
            results = [
                {"index": 2, "delta": 3, "hexworld": "https://hexworld.org/board/#21c1,k10k11", "output_json": str(path_a), "ok": True},
                {"index": 1, "delta": 12, "hexworld": "https://hexworld.org/board/#21c1,k11", "output_json": str(path_b), "ok": True},
            ]
    ...
    def test_main_runs_targets_and_writes_manifest_and_square_page(self):
        with TemporaryDirectory() as td:
            td_path = Path(td)
            args = SimpleNamespace(workers=2, catalog=None, delta=None, out_dir=str(td_path / "batch"))
            targets = (
                pattern_study_batch.StudyTarget(delta=12, hexworld="https://hexworld.org/board/#21c1,k11"),
                pattern_study_batch.StudyTarget(delta=3, hexworld="https://hexworld.org/board/#21c1,k10k11"),
            )
