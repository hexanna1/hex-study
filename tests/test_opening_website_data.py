...
class OpeningWebsiteDataTests(unittest.TestCase):
    ...
    def _decode_bundle(self, payload: bytes) -> dict[str, object]:
        ...
        move_stream_offset = offset
        move_bit_length = candidate_count * move_id_bits
        prior_stream_offset = move_stream_offset + ((move_bit_length + 7) // 8)
        prior_bit_length = candidate_count * owd.PACKED_CANDIDATE_METRIC_BITS
        delta_stream_offset = prior_stream_offset + ((prior_bit_length + 7) // 8)
        delta_bit_length = sum(count * owd.PACKED_CANDIDATE_DELTA_BITS for _is_core, count in nodes if count > 1)
        child_stream_offset = delta_stream_offset + ((delta_bit_length + 7) // 8)
        child_bit_length = sum(1 for _is_core, count in nodes if count > 0)
        exception_offset = child_stream_offset + ((child_bit_length + 7) // 8)
        candidates = []
        node_offset = 0
        candidate_index = 0
        move_bit_offset = 0
        ...
        def walk(parent_edge_red: int | None, ply: int) -> None:
            nonlocal node_offset, candidate_index, move_bit_offset, prior_bit_offset, delta_bit_offset, child_bit_offset
            ...
            if count > 0:
                has_child = bool(
                    self._read_bits(
                        payload,
                        offset=child_stream_offset,
                        bit_offset=child_bit_offset,
                        bits=1,
                    )
                )
                child_bit_offset += 1
            for _ in range(count):
                move_id = self._read_bits(
                    payload,
                    offset=move_stream_offset,
                    bit_offset=move_bit_offset,
                    bits=move_id_bits,
                )
                move_bit_offset += move_id_bits
                prior = self._read_bits(
                    payload,
                    offset=prior_stream_offset,
                    bit_offset=prior_bit_offset,
                    bits=owd.PACKED_CANDIDATE_METRIC_BITS,
                )
                prior_bit_offset += owd.PACKED_CANDIDATE_METRIC_BITS
                if count == 1:
                    red = parent_edge_red
                else:
        ...
        self.assertEqual(move_bit_offset, move_bit_length)
        self.assertEqual(prior_bit_offset, prior_bit_length)
        self.assertEqual(delta_bit_offset, delta_bit_length)
        self.assertEqual(child_bit_offset, child_bit_length)
    ...
    def test_build_opening_bundle_rejects_mixed_child_presence(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaisesRegex(ValueError, "mixed child presence"):
