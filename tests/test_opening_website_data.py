...
class OpeningWebsiteDataTests(unittest.TestCase):
    def _decode_bundle(self, payload: bytes) -> dict[str, object]:
        ...
        for _ in range(node_count):
            raw_parent = int.from_bytes(payload[offset:offset + 3], "little")
            move_id, flags = owd.NODE_TAIL_STRUCT.unpack_from(payload, offset + 3)
            nodes.append(
                [
                    (-1 if raw_parent == 0 else raw_parent - 1),
                    move_id,
                    bool(flags & 0x80),
                    flags & 0x7F,
                ]
