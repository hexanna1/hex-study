...
class StudyCommonTests(unittest.TestCase):
    def test_native_move_normalization_applies_all_triangle_orientations(self):
        rows = study_common._run_native_move_normalization(
            requests=[
                ("https://hexworld.org/board/#11c1,a3f6b3g6", ["a2", "b2", "c2"]),
                ("https://hexworld.org/board/#11c1,k9f6j9g6", ["k10", "j10", "i10"]),
                ("https://hexworld.org/board/#11c1,f6c1g6c2h6", ["b1", "b2", "b3"]),
                ("https://hexworld.org/board/#11c1,f6i3g6", ["j3", "j2"]),
            ],
            board_size=11,
        )
        self.assertEqual(
            rows,
            [["c2"] * 3, ["i10"] * 3, ["b3"] * 3, ["j2"] * 2],
        )

    def test_native_move_normalization_requires_anchor_color_to_move(self):
        rows = study_common._run_native_move_normalization(
            requests=[
                ("https://hexworld.org/board/#11c1,a3", ["a2", "b2"]),
            ],
            board_size=11,
        )
        self.assertEqual(rows, [["a2", "b2"]])
    ...
    def test_candidate_key_local_roundtrip_for_multiple_transforms(self):
        ...
        for transform_id in (0, 1, 7):
            ...
            exp_meta = {
                ...
                "board_offset": [offset[0], offset[1]],
            }
