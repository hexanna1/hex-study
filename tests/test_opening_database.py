...
class OpeningDatabaseTests(unittest.TestCase):
    def test_opening_expansion_uses_native_move_normalization(self):
        position = "https://hexworld.org/board/#11c1,a3f6"
        candidates, _proofs = od._run_opening_expansion(
            nodes=[od.OpeningNode(position=position, ply=2)],
            payloads={
                position: {
                    "m": [["a2", 500000], ["b2", 400000], ["c5", 300000]],
                }
            },
            board_size=11,
        )
        self.assertEqual([row["move"] for row in candidates[0]], ["b2", "c5"])
