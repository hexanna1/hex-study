...
The opening database is line-based with position reuse.

Each node corresponds to:

- one ordered whole-board move sequence,
- plus the realized whole-board position reached by that sequence.

The realized position is the expansion state and the evaluation/cache key.
Different move orders that reach the same position may reuse the same analyzed position payload, but they still remain different opening nodes in the retained study tree.
