...
The opening database is a position-keyed DAG.

Each node corresponds to:

- one realized whole-board position,
- plus one retained parent and move for display and navigation.

The realized position is the expansion state and the evaluation/cache key.
Different move orders that reach the same position share one opening node, retaining the greatest incoming importance.

### Joseki

The joseki database is a position-keyed DAG that also tracks whether the previous ply was tenuki.

Each node corresponds to:

- one realized position inside a fixed family-specific balanced context,
- whether the previous ply was tenuki,
- plus one retained corner-local line for display and navigation.

The realized position and previous-tenuki state form the node identity.
Different lines that transpose to the same identity share one joseki node, retaining the greatest incoming importance.
...
- openings use a small whole-board top-`k` policy, with a programmatically derived fair-opening root set and a small curated set of root-level importance exceptions where practical study coverage would otherwise be skewed,
- joseki uses local-corner continuations plus an optional tenuki continuation, with a curated family-specific root set and no retained second consecutive tenuki.
...
These scores are used locally for pruning and child comparison.

For opening website display, retained continuations may also be summarized into a propagated tree winrate by averaging
child winrates in proportion to retained policy mass. This is intended as a coherent study value over the retained
tree rather than a strict best-reply value.
...
- allow a small penalized tail beyond the main retained set where useful for study coverage, but do not let that tail define the local baseline for continuation quality,
