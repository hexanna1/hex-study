...
- In candidate mode, treat time budget as total across all candidates.
- Practical heuristic:
  - choose `search_seconds` proportional to candidate count,
  - and scale seconds-per-candidate by board size (smaller boards generally need less time per candidate).
  - example defaults:
    - around `1 * number_of_candidates` seconds on 19x19 proxy sweeps,
    - around `2 * number_of_candidates` seconds on 21x21 confirmation sweeps.
